  ...
  list: [
    {
      ...
      descriptions: [
        {
          content:
            '광고주가 토스 광고 성과를 측정하는 자사 웹 전환 픽셀·앱 MMP(AppsFlyer, Adjust 등) 연동이 누락·오류나면 실제 토스 광고로 일어난 전환이 기여로 인정되지 않는 문제를 풀기 위해 신규 플랫폼 설계, 팀 리드·서버·PO 겸임',
        },
        {
          content:
            '기존 앱 로그 검증 엔진(로그 정의, 파라미터, 정답지 모델)을 광고 전환 도메인으로 확장 재사용',
          descriptions: [
            {
              content: '광고 전용으로는 Hard(실패)/Soft(경고) 심각도 축만 추가해 판정 분기',
            },
          ],
        },
        {
          content: '검수 대상 자동 인입 서버 설계',
          descriptions: [
            {
              content:
                '소재 승인, 카탈로그 상품세트의 캠페인 연결 이벤트를 Kafka로 구독해 검수 대상 자동 생성',
            },
            {
              content:
                '판정 결과를 가르는 축(도메인, 추적 방식, 광고 목표, 웹/앱)을 dedup 키로 묶어 같은 기준의 대상은 한 번만 검수',
            },
            {
              content: '인입 실패는 자체 dead letter 큐로 재처리',
            },
          ],
        },
        {
          content: '검수 추적키 기반 로그 격리 설계',
          descriptions: [
            {
              content: '검수 시작 시 예측 불가능한 고유 키를 발급해 광고 클릭 ID(TCID)에 실어 전송',
            },
            {
              content:
                '이 키가 광고주 전환 로그까지 따라가, 실제 고객 트래픽 속에서 이번 검수의 로그만 조인 가능',
            },
          ],
        },
        {
          content: '광고 목표·하위 유형별 동적 정답지 판정 엔진 개발',
          descriptions: [
            {
              content:
                '광고 목표와 하위 유형에 따라 기대 이벤트와 파라미터 규칙을 런타임에 동적으로 구성',
            },
            {
              content: '여러 이벤트 중 하나만 와도 되는 택일 조건 지원',
            },
          ],
        },
        {
          content: '검수자가 실기기 미러링으로 광고 클릭부터 구매·설치까지 직접 수행(HITL)',
          descriptions: [
            {
              content:
                '검수자 1인당 기기 1대 고정 매핑이 기본, 기기 상태가 소진되는 유형(앱 설치 광고)만 기기 풀에서 자동 배정',
            },
          ],
        },
        {
          content: '미입수 원인 분류 로직 설계',
          descriptions: [
            {
              content:
                '픽셀·SDK 미검증 / 이벤트 미발송 / MMP 포스트백 미수신으로 나눠 광고주에게 정확한 원인 안내',
            },
            {
              content:
                '검수 시간 창 안에 이번 회차와 연결되지 않은 전환이 있었는지 함께 보여줘 추적키 유실과 미발송을 구분',
            },
          ],
        },
        {
          content: '상태 관리와 워치독 설계',
          descriptions: [
            {
              content: '진행, 검증 중, 판정, 미입수, 만료, 검수 불가 상태를 정의',
            },
            {
              content:
                '검증 중 서버가 끊겨도 재개 예산(기본 30분) 안에서는 검증을 이어가고, 넘으면 미입수로 종결',
            },
            {
              content:
                '검수 수단이 없는 경우(랜딩 종료, 품절)는 검수 불가로 분리해 커버리지 분자·분모 양쪽에서 제외',
            },
          ],
        },
        {
          content:
            '광고 프로덕트·캠페인·데이터(DMP)·정책심사·기기 자동화 팀과 판정 기준을 단일 문서로 확정',
          descriptions: [
            {
              content:
                '카탈로그 검수 방향(인입 시점, 상품 조회 방식, 샘플링 소유)은 여러 조직 협의로 결정',
            },
          ],
        },
        {
          content:
            '2026.08.21 라이브 오픈, 첫 주부터 실광고주 통화·금액·전환 추적 코드 위반 첫 검출',
          weight: 'MEDIUM',
        },
        {
          content:
            '흩어진 임시 검수 도구 흡수, 검수 순서를 전환 검수 → 소재 검수로 확정, 사전 검수까지 역할 확대',
        },
        {
          content: '다음 과제로 광고 노출·클릭·입찰 로그 검증 설계 착수 (2026.09~)',
        },
      ],
    },
    {
      ...
      descriptions: [
        {
          content:
            '토스 앱 로그 검증을 실기기 자동 조작 + 실시간 로그 대조 방식으로 전환, 팀 출범 첫날부터 서버 전담 후 팀 리드로 성장',
        },
        {
          content: 'PoC 범위를 로그 20종, 시나리오 5개로 좁혀 2주 만에 핵심 구조를 통과',
          descriptions: [
            {
              content:
                '원격 실행(기기 자동화 플랫폼 연동), 시나리오 수행, 서버 판정 로직 3가지 검증',
            },
            {
              content:
                '데이터 웨어하우스 SQL 조회 방식은 적재 지연으로 즉시성이 없어 폐기, 실기기 실시간 수집으로 방향 전환',
            },
          ],
        },
        {
          content: '통합 어드민 → 전용 어드민 → 캔버스(DAG) 기반 시나리오까지 3세대 제품 고도화',
          descriptions: [
            {
              content:
                '캔버스는 흐름을 노드·엣지로 그리면 가능한 모든 경로를 자동으로 뽑아 병렬 실행',
            },
            {
              content: '경로는 해시로 식별해, 그래프를 고쳐도 기존 경로 이력을 이어감',
            },
          ],
        },
        {
          content: 'AI 정답지 기능 설계 및 개발',
          descriptions: [
            {
              content:
                '같은 시나리오를 여러 라운드 돌려 모은 실제 로그 값을 LLM(Flowise)이 분석해 정답지 초안과 비교 방식(고정값/형식/범위/존재 여부)을 제안',
            },
            {
              content:
                '로그 명세에 신규 파라미터가 생기면 동기화 배치가 감지해 정답지 초안 생성까지 자동 실행',
            },
            {
              content: '초안은 "시스템 제안" 상태로만 두고 사람이 승인해야 반영',
            },
          ],
        },
        {
          content: '오토매핑, 자동 복구 연동, A/B 실험군 검수로 자동화 계층 확장',
          descriptions: [
            {
              content:
                '오토매핑: 노드를 실제로 실행해 찍힌 로그로 검증 대상을 자동 연결, 실행 추적 ID와 세션 ID를 분리해 서버 간 판단 불일치 제거',
            },
            {
              content:
                '자동 복구 연동: 화면이 바뀌어 스텝이 깨지면 실패 경로와 의도를 자연어 UI 자동화 도구에 넘겨 고치고 사람이 승인',
            },
            {
              content:
                'A/B 실험군 검수: 검수 계정을 특정 실험 변형에 강제로 할당해 실험 지면까지 검수',
            },
          ],
        },
        {
          ...
          descriptions: [
            {
              content:
                '배포·재시작 시 실행이 유실되던 fire-and-forget 구조를 DB 선기록 → Kafka → 소비자 실행 구조로 전환',
            },
            {
            ...
            {
              content:
                'Outbox·CDC는 팀 규모 대비 과하다고 판단해 배제, 기능 플래그로 되돌릴 수 있게 설계',
            },
          ],
        },
        {
          ...
          descriptions: [
            {
              content:
                '"로그 제공자" 개념으로 형식 등록부, 대량 등록, 저장소 내 로그 식별자 격리를 설계해 검증 범위 확장',
            },
          ],
        },
        {
          content: '플랫폼 서버에 MCP 서버 내장',
          descriptions: [
            {
              content:
                '읽기 전용 진단 도구로 시작해 시나리오 구성부터 결과 해석까지 대화형으로 다룰 수 있는 수준까지 확장',
            },
            {
        ...
        {
          content:
            '검증 방식을 출시 직전 수동 확인에서 하루 2회 자동 검수로 전환, 커버리지 70%대 확보',
          weight: 'MEDIUM',
        },
        {
          content:
            '커머스 검색 조직 로그 버그 방어 계획 편입, 경영진 주간 회의 커버리지 정기 보고, 코어 얼라인먼트 데이(2026.07) 전사 발표',
        },
        {
