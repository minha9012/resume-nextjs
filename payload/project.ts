  ...
  list: [
    {
      ...
      descriptions: [
        {
          content:
            '광고주 전환 측정 연동(픽셀, MMP) 누락·오류로 광고 성과가 부당하게 저평가되는 문제를 풀기 위해 신규 플랫폼 설계, 팀 리드·서버·PO 겸임',
        },
        {
          content:
            '기존 앱 로그 검증 엔진을 광고 전환 도메인으로 확장 재사용, Hard/Soft 심각도 축 추가',
        },
        {
          content:
            '소재 승인, 카탈로그 캠페인 연결 이벤트를 Kafka로 받아 검수 대상 자동 인입, 판정 축이 같은 대상은 중복 제거',
        },
        {
          content:
            '검수자가 실기기 미러링으로 광고 클릭부터 구매·설치까지 직접 수행(HITL), 추적키로 실제 트래픽 속 검수 로그만 격리해 조인',
        },
        {
          content:
            '광고 목표·하위 유형별 정답지 엔진 설계, 미입수 원인은 픽셀·SDK 미검증 / 이벤트 미발송 / 포스트백 미수신으로 분류',
        },
        {
          content:
            '검증 중 서버 장애는 재개 예산으로, 검수 불가 유형은 커버리지 분자·분모에서 함께 제외',
        },
        {
          content:
    ...
    },
    {
      ...
      descriptions: [
        ...
        {
          content:
            'PoC 범위를 로그 20종, 시나리오 5개로 좁혀 2주 만에 원격 실행·시나리오 수행·서버 판정 통과',
        },
        {
          content: '데이터 웨어하우스 SQL 방식은 적재 지연으로 폐기, 실기기 실시간 수집으로 전환',
        },
        {
        ...
        {
          content:
            '여러 라운드 실행으로 모은 로그 값을 LLM(Flowise)으로 분석해 정답지 초안과 비교 방식을 제안하는 AI 정답지 기능 개발, 신규 파라미터 자동 감지',
        },
        {
          content: '정답지 초안은 "시스템 제안" 상태로 두고 사람이 승인해야 반영',
        },
        {
          content:
            '오토매핑, 자동 복구 연동(자연어 UI 자동화 도구), A/B 실험군 검수로 자동화 계층 확장',
        },
        {
          content:
            '비동기 실행을 DB 선기록 → Kafka → 소비자 실행 + 기동 복구 + 좀비 워치독 구조로 전환',
        },
        {
          content:
            '앱 로그를 넘어 결제·인증 등 서버 도메인 로그까지 "로그 제공자" 개념으로 검증 대상 일반화',
        },
        {
          content:
            '플랫폼 서버에 MCP 내장, AI 에이전트가 기능을 직접 호출해 시나리오 구성부터 결과 해석까지 대화로 처리',
        },
        {
          content:
